...
let findAverage: GetNumber = (data) => {
  // let a and b the nums but b is getting the age
...
let findCAOverN: GetVolunteers = (data, minAge) => {
  return data.filter(i => i.state === "CA" && i.age >= minAge);
...
let daBigTest: GetVolunteer = (data) => // Code here
{
  let vol: Volunteer | undefined;
  vol = data.find(user => user.city === "SLO" && user.age > 40);
  if (vol)
  {
    // position
    vol.position = "staff";
