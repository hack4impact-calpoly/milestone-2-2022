...
let findAverage: GetNumber = (data) => {
  const ageList: number[] = data.map((user) => user.age);
  const sum: number = ageList.reduce((a, b) => a + b);
...
let findIndexAns: GetNumber = (data) =>
  data.findIndex((user) => user.city === 'SF');
...
let findCAOverN: GetVolunteers = (data, minAge) =>
  data.filter((user) => user.state === 'CA' && user.age > minAge);
...
console.log('kyleClone', kyleClone);
...
let daBigTest: GetVolunteer = (data) => {
  let user = data.find((user) => user.city === 'SLO' && user.age > 40);
