...
let findAverage:GetNumber=(data)=>data.reduce((acc, cur)=>acc+cur.age, 0) / data.length; // Define lambda function here
...
*/
let findIndexAns: GetNumber =(data)=>data.findIndex(d=>d.city==='SF'); // Code here
...
let findCAOverN: GetVolunteers=(data, minAge)=>data.filter(d=>d.age>minAge && d.state=='CA' ); // Code here
...
let findSBStaff: GetVolunteer=data=>data.find(d=>d.city==='SB'); // Code here
...
let updateVolunteer: UpdateVolunteer = (data,newData) => {
  return {...data, ...newData};
};// Code here
...
let daBigTest: GetVolunteer=vol=>{
  const p=vol.find(v=>v.city==='SLO' && v.age>40)
  if(p) p.position='staff'
  return p
}; // Code here
