...
const userData: Volunteer[] = [
  ...
  //gotird of a comma above this line
];
...
let findAverage: GetNumber = (data) =>{
  //create array containing only age
  ...
  return total/data.length;
   //return data.map(x => x.age).reduce((previousValue, currentValue) => previousValue + currentValue, 0)/data.length
};
...
let findIndexAns: GetNumber = data =>{
  return data.findIndex(element => element.city === "SF");
};
...
let findCAOverN: GetVolunteers = (data, minAge) =>{
   ...
   return data.filter(x => x.age > minAge && x.state == "CA");
};
...
let findSBStaff: GetVolunteer = data =>{
  return data.find(element => element.city === "SB");
};
...
//got rid of the second let and added the location parameter
...
let daBigTest: GetVolunteer = (data) =>{
    let updated_pos: Partial<Volunteer> = {position: "staff"};
    let status = data.find(element => (element.city === "SLO" && element.age > 40));
