...
let findAverage: GetNumber = (data: Volunteer[]): number => {
  let sum = 0;
...
*/
type getCity = (data: Volunteer[]) => getCity;
let findIndexAns: GetNumber = (data: Volunteer[]): number => {
  for (let i = 0; i < data.length; i++){
    console.log(data[i].city);
    if (data[i].city === "SF") return i;
  }
  return -1;
}
...
let findCAOverN: GetVolunteers = (userData: Volunteer[], n: number): Volunteer[] => {
  return userData.filter((volunteer: Volunteer) => volunteer.age > n && volunteer.state === "CA");
}
...
let findSBStaff: GetVolunteer = (userData: Volunteer[]): Volunteer | undefined =>{
...
let daBigTest: GetVolunteer = (volLst: Volunteer[]): Volunteer | undefined =>  {
  for (let i = 0; i < volLst.length; i++){
