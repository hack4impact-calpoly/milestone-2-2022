...
*/
let findIndexAns: GetNumber = (data) => data.findIndex((volunteer) => (volunteer.city == "SF")); // Code here
...
let findCAOverN: GetVolunteers = (data, minAge) => data.filter((volunteer) => (volunteer.state == "CA" && volunteer.age > minAge)); // Code here
...
let daBigTest: GetVolunteer = (data) => {
  let sloOldPerson = data.find((vol) => (vol.city == "SLO" && vol.age > 40));

  if (sloOldPerson == undefined) {
    return undefined;
  }

  return Object.assign(sloOldPerson, {position: "staff"})
}; // Code here
