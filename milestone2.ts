...
let findCAOverN: GetVolunteers = (data, minAge) => {
  return data.filter(person => person.state === "CA" && person.age > minAge );
...
let updateVolunteer: UpdateVolunteer=(vol: Volunteer, update:Partial<Volunteer>)=>{
  return {...vol, ...update}
} // Code here
...
let getVolunteerInfo: GetVolunteerInfo = (vol) => { // Code here
