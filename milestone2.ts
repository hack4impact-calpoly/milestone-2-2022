...
*/
type GetCity = (data: Volunteer[]) => number;
let findIndexAns: GetCity = (data) =>{ 
  //data.findIndex(city => city === "SF");
  const noSF = -1;
  for(let i = 0; i < data.length; i++){
    if (data[i].city === "SF"){
      return i;
...
let findCAOverN: GetVolunteers = (data, minAge) => 
  ...
  data.filter(x => x.state === "CA" && x.age > minAge);
...
let daBigTest: GetVolunteer = (data) => {
  let findSLOand40 = data.find(x => x.city === "SLO" && x.age > 40);
  
  //let updatedPosition: Partial<Volunteer> = {position: 'staff'};
  if (findSLOand40){
    return {...findSLOand40, position: "staff"}
  }
  return undefined;
  }
