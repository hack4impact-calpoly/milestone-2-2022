...
let findAverage: GetNumber = data => data.reduce((acc, curr) => acc + curr.age, 0) / data.length; // Define lambda function here
...
*/
let findIndexAns: GetNumber = data => data.map(e => e.city).indexOf("SF"); // Code here
...
let findCAOverN: GetVolunteers = 
  (data, minAge) => data.filter(e => e.state === "CA" && e.age > minAge); // Code here
...
let findSBStaff: GetVolunteer = (data) => data.find(e => e.city === "SB"); // Code here
...
let daBigTest: GetVolunteer = (data) =>
  data.find(e => e.city === 'SLO' && e.age > 40) ? 
  {...(data.find(e => e.city === 'SLO' && e.age > 40)) as Volunteer, position: "staff"} :
  undefined; // Code here
