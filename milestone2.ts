...
let findAverage: GetNumber = (data) => {
  let res: number = 0;
  data.forEach((person: Volunteer) => res += person.age);
...
let findIndexAns: GetNumber = (data) => {
  return data.findIndex((person: Volunteer) => person.city === "SF");
};
...
let findCAOverN: GetVolunteers = (data, minAge) => {
  return data.filter((person: Volunteer) => person.state === "CA" && person.age > minAge);
}; // Code here
...
let findSBStaff: GetVolunteer = (data) => {
  return data.find((person: Volunteer) => person.city === "SB");
}; // Code here
...
let daBigTest: GetVolunteer = (data) => {
  let person =  data.filter((vol: Volunteer) => vol.age > 40).find((vol: Volunteer) => vol.city === "SLO")
