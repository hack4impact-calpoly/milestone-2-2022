...
let copyVolunteer: CopyVolunteer = (vol) => {
  const volunteerClone: Volunteer = {...vol}
  return volunteerClone;
}
...
let updateVolunteer: UpdateVolunteer = (vol, updatedLocation) => {
  const kyleNew: Volunteer = {...vol, ...updatedLocation}
  return kyleNew
};
...
let daBigTest: GetVolunteer = (data) => {
  for (var volunteer of data) {
    if (volunteer.city == "SLO" && volunteer.age > 40) {
      const volunteerCopy = copyVolunteer(volunteer);
      const result = updateVolunteer(volunteerCopy, { position: "staff" });
      return result;
    }
  }
  return undefined;
