...
*/
const isSF = (element: String) => element === "SF";
let findIndexAns: GetNumber = (data) => data.map(v => v.city).findIndex(isSF); // Code here
...
let findCAOverN: GetVolunteers = (data, minAge) => data.filter(v => v.age > minAge && v.state === "CA"); // Code here
...
let findSBStaff: GetVolunteer = (data) => data.find(v => v.city === "SB"); // Code here
...
let copyVolunteer: CopyVolunteer = (vol) => {return {...vol}}; // Code here
...
let updateVolunteer: UpdateVolunteer = (vol, updates) => {return {...vol, ...updates}}; // Code here
...
let getVolunteerInfo: GetVolunteerInfo = (vol) => {
  let {name, age, city} : {name: string, age: number, city: string} = vol; // Code here
...
let daBigTest: GetVolunteer = (data) => {
  let person = data.filter((v: Volunteer) => v.age > 40).find((v: Volunteer) => v.city === "SLO");
  return person ? {...person, position: "staff"} : undefined;
};
