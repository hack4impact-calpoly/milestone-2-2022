...
*/
let findIndexAns: GetNumber = data => data.findIndex(vol => vol.city === 'SF'); // Code here
...
let findCAOverN: GetVolunteers = (data: Volunteer[], minAge: number) => data.filter(vol =>  vol.state === "CA" && vol.age >= minAge); // Code here

// console.log(findCAOverN(userData, 0));
...
let findSBStaff: GetVolunteer = data => data.find(vol => vol.city === 'SB'); // Code here
...
let updateVolunteer: UpdateVolunteer = (vol: Volunteer, updates: Partial<Volunteer>) => {return {...vol, ...updates}}; // Code here
