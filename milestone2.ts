...
let findAverage: GetNumber = (data) => {
  return data.reduce((p,c) => p + c.age,0)/data.length;
}
...
{
  return data.filter((element) => element.state==="CA" && element.age > minAge);
...
let updateVolunteer: UpdateVolunteer = (vol,update) =>
{
  vol = {...vol,...update};
  return vol;
}
...
{
  let person = vol.find(el => el.city === "SLO" && el.age > 40);
  if(person) person.position="staff";
  return person;
}
