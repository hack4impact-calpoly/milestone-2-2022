...
*/
let isFromSf = (volunteer: Volunteer) => {
  return volunteer.city === "SF";
}

let findIndexAns: GetNumber = (data) => {
  return data.findIndex(isFromSf);
}
...
let getVolunteerInfo: GetVolunteerInfo = (vol) => {
  let name = vol.name, age = vol.age, city = vol.city; // Code here
...
let daBigTest: GetVolunteer = (data) => {
  console.log(data)
  let overFourty = data.find(vol => vol.city === "SLO" && vol.age > 40);
  return (!overFourty ? undefined : {
    ...overFourty,
    position: "staff"
  })
}
