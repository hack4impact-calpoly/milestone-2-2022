...
let findCAOverN: GetVolunteers = (data: Volunteer[], minAge: number) => {
  return data.filter(v => v.state === "CA" && v.age > minAge);
};
...
let daBigTest: GetVolunteer = (data: Volunteer[]) => { 
  let volIndex = data.findIndex(v => v.city === "SLO" && v.age > 40);
  if (volIndex !== -1) {
    let updatedPosition: Partial<Volunteer> = {position: "staff"};
    let newStaff: Volunteer = {...data[volIndex], ...updatedPosition};
    return newStaff;
  }
  else {
    return undefined;
